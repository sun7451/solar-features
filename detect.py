...
def detect_filament(pngfile, ifcheck):
...
# -------------------------------------------------
name = './images/20150301070034Lh.png'  # unclear
...
labelim, result, corrected = detect_filament(name, ifcheck=1)
...
for region in regionprops(labelim, intensity_image=corrected):
    if region.area >= 10:
        ...
        print('centroid,area,length,width=', region.centroid, region.area_filled, region.axis_major_length,
        ...
        fila = json.dumps({'fitsID': str(name),
                           'filament_ID': id,
                           'centroid': str(region.centroid),
                           'area': str(region.area_filled),
                           'length': str(region.axis_major_length),
                           'width': str(region.axis_minor_length),
                           'outlines': str(region.coords)}, \
                          sort_keys=False, indent=4, separators=(',', ': '))
        id = id + 1
        # if id==1:
filament.append(fila)
print(filament)
ax.set_axis_off()
plt.tight_layout()
plt.show()
